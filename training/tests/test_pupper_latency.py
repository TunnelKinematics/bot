...
def test_lag_resampled_every_step_not_fixed_per_episode():
    ...
    for v in range(2, 12):
        cur = (
            buf.step(torch.full((4096, 1), float(v)))[:, 0] == float(v)
        ).float()
...
def test_reset_clears_history_per_env():
    buf = _buffer(
        (0.0, 1.0), num_envs=4
    )  # always lag 1, so we read the history
    ...
    try:
        ...
        for _ in range(60):
            ...
            if prev_imu is not None:
                stale += (
                    ((frame[:, :6] - prev_imu).abs().max(dim=1).values < 1e-12)
                    .float()
                    .mean()
                )
            prev_imu = frame[:, :6]
...
def test_deterministic_latency_is_the_default_and_does_not_leak():
    ...
    assert act.delay_min_lag == act.delay_max_lag == n, (
    ...
    assert PUPPER_ACTUATOR.delay_min_lag == 0, (
        "leaked delay into the shared actuator"
    )
    assert PUPPER_ACTUATOR.delay_max_lag == 0, (
        "leaked delay into the shared actuator"
    )
...
def test_critic_sees_applied_action_actor_does_not():
    ...
    try:
        env.reset()
        assert (
            "applied_action" in env.observation_manager.active_terms["critic"]
        )
        ...
        for _ in range(40):
            ...
            lagged += float(
                (~torch.isclose(applied, processed).all(dim=1)).float().mean()
            )
...
def test_airborne_pretrain_isolates_gait_tracking_then_releases():
    ...
    try:
        ...
        assert live == {"gait_tracking"}, (
            f"warmup should isolate the gait, got {live}"
        )
        ...
        after = {
            n for n in rm.active_terms if rm.get_term_cfg(n).weight != 0.0
        }
        assert after > live, (
            "releasing the warmup must restore the other terms"
        )
