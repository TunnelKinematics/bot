...
from mjlab.tasks.pupper_gait.mdp.gait_reference import (
    build_joint_reference_table,
)
...
def _actor() -> _Actor:
    ...
    actor = _Actor().eval()
    with torch.no_grad():
        cast(torch.Tensor, actor.obs_normalizer._mean).copy_(
            torch.randn(1, IN_DIM)
        )
        cast(torch.Tensor, actor.obs_normalizer._std).copy_(
            torch.rand(1, IN_DIM) + 0.5
        )
    return actor
...
def _gait_block() -> dict:
    return {
        ...
        "gallop_table": build_joint_reference_table(
            N_SAMPLES, "gallop"
        ).tolist(),
    }
...
def _commands() -> np.ndarray:
    ...
    cases = [
        ...
        (
            ...
            0.0,
            0.8,
        ),  # turn left, |vx| under the direction threshold.
        ...
        (
            ...
            0.01,
            0.0,
        ),  # tiny command: blend still saturated by norm/0.1.
        ...
        (
            ...
            0.0,
            0.0,
        ),  # long uptime (no episode resets on the robot).
    ]
    cases += [
        tuple(x)
        for x in rng.uniform(
            [-0.0, -1.0, -0.5, -2.0], [50.0, 1.0, 0.5, 2.0], size=(32, 4)
        )
    ]
...
def test_lift_table_plays_when_not_translating():
    ...
    lifted = reference_offset_numpy(
        _mixed_gait_block(), default, t, vx, vy, yaw
    )
...
def test_lift_table_does_not_touch_translating_commands():
    ...
    vy = np.zeros(3)
    yaw = np.zeros(3)
    with_lift = reference_offset_numpy(
        _mixed_gait_block(), default, t, vx, vy, yaw
    )
...
def test_fast_back_table_plays_only_backward_fast():
    ...
    split = reference_offset_numpy(
        _direction_split_block(), default, t, vx, vy, yaw
    )
    merged = reference_offset_numpy(
        _mixed_gait_block(), default, t, vx, vy, yaw
    )
...
def test_fast_back_equal_to_forward_is_inert():
    ...
    without = reference_offset_numpy(
        _mixed_gait_block(), default, t, vx, vy, yaw
    )
