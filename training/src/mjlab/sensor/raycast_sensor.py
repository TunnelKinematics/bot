...
class GridPatternCfg:
    ...
    def generate_rays(
        ...
        direction = torch.tensor(
            self.direction, device=device, dtype=torch.float32
        )
...
class PinholeCameraPatternCfg:
    ...
    def generate_rays(
        ...
        local_directions = local_directions / local_directions.norm(
            dim=1, keepdim=True
        )
...
class RayCastSensorCfg(SensorCfg):
    ...
    class VizCfg:
        ...
        hit_sphere_color: tuple[float, float, float, float] = (
            0.0,
            1.0,
            1.0,
            1.0,
...
class RayCastSensor(Sensor[RayCastData]):
    ...
    def __init__(self, cfg: RayCastSensorCfg) -> None:
        ...
        self._frame_infos: list[
            tuple[Literal["body", "site", "geom"], int, int]
        ] = []
        self._num_frames: int = 0
        self._num_rays_per_frame: int = 0

        self._local_offsets: torch.Tensor | None = None
        self._local_directions: torch.Tensor | None = (
            None  # [rays_per_frame, 3]
        )
    ...
    def initialize(
        ...
        self._ray_bodyexclude = wp.array(
            body_excludes, dtype=int, device=device
        )
        ...
        self._normals_w = torch.zeros(
            num_envs, self._num_rays, 3, device=device
        )
                ...
                for i in range(N):
                    ...
                    if hit:
                        ...
                        if self.cfg.viz.show_normals:
                            normal_end = (
                                end + normals[k, ray_idx] * normal_length
                            )
                            visualizer.add_arrow(
                                start=end,
                                end=normal_end,
    ...
    def prepare_rays(self) -> None:
        ...
        assert (
            self._local_offsets is not None
            and self._local_directions is not None
        )
        ...
        world_offsets = torch.einsum(
            "bfij,nj->bfni", rot_mat, self._local_offsets
        )
        world_origins = frame_pos[:, :, None, :] + world_offsets
        world_rays = torch.einsum(
            "bfij,nj->bfni", rot_mat, self._local_directions
        )
    ...
    def postprocess_rays(self) -> None:
        ...
        self._hit_pos_w = (
            ...
            + self._cached_world_rays * clamped.unsqueeze(-1)
        )
    ...
    def _compute_alignment_rotation(
        self, frame_mat: torch.Tensor
    ) -> torch.Tensor:
        ...
        if self.cfg.ray_alignment == "base":
            ...
            raise ValueError(
                f"Unknown ray_alignment: {self.cfg.ray_alignment}"
            )
