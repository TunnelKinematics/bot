...
class ContactSensor(Sensor[ContactData]):
    ...
    def __init__(self, cfg: ContactSensorCfg) -> None:
        ...
        if (
            cfg.global_frame
            and cfg.reduce != "netforce"
            and ("normal" not in cfg.fields or "tangent" not in cfg.fields)
        ):
            raise ValueError(
        ...
        if self.cfg.track_air_time:
            ...
            self._air_time_state = _AirTimeState(
                current_air_time=torch.zeros(
                    (n_envs, n_primary), device=device
                ),
    ...
    def _compute_data(self) -> ContactData:
        ...
        if self._air_time_state is not None:
            ...
            out.current_contact_time = (
                self._air_time_state.current_contact_time
            )
    ...
    def compute_first_contact(
        self, dt: float, abs_tol: float = 1.0e-6
    ) -> torch.Tensor:
    ...
    def _extract_sensor_data(self) -> ContactData:
        ...
        field_chunks: dict[str, list[torch.Tensor]] = {
            f: [] for f in self.cfg.fields
        }
    ...
    def _update_air_time_tracking(self, dt: float) -> None:
        ...
        if self.cfg.num_slots > 1:
            found = found.view(found.size(0), -1, self.cfg.num_slots).any(
                dim=-1
            )
        is_contact = found > 0
    ...
    def _update_history(self) -> None:
        ...
        if "force" in self._history_state and contact_data.force is not None:
            self._history_state["force"] = self._history_state["force"].roll(
                1, dims=2
            )
        ...
        if "dist" in self._history_state and contact_data.dist is not None:
            self._history_state["dist"] = self._history_state["dist"].roll(
                1, dims=2
            )
    ...
    def _resolve_primary_names(
        ...
        if match.mode == "geom":
            ...
            raise ValueError(
                "Primary mode must be one of {'geom','body','subtree'}"
            )
        ...
        if excludes:
            ...
            if exclude_patterns:
                names = [
                    n
                    for n in names
                    if not any(rx.search(n) for rx in exclude_patterns)
                ]
    ...
    def _add_contact_sensor_to_spec(
        ...
        if secondary_name is not None:
            ...
            kwargs["refname"] = _prefix_name(
                secondary_name, self.cfg.secondary.entity
            )
