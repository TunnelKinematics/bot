...
class BuiltinSensorCfg(SensorCfg):
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            and self.sensor_type not in _SENSORS_ALLOWING_REF
        ):
            raise ValueError(
    ...
    def edit_spec(
        ...
        for sensor in scene_spec.sensors:
            if sensor.name == self.cfg.prefixed_name:
                is_entity_scoped = (
                    self.cfg.obj is not None
                    and self.cfg.obj.entity is not None
                )
                if is_entity_scoped:
                    raise ValueError(
