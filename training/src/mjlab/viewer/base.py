...
class BaseViewer(ABC):
    ...
    def _handle_custom_action(
        self, action: ViewerAction, payload: Any | None
    ) -> bool:
        del action, payload
        return False
    ...
    def request_action(self, name: str, payload: Any | None = None) -> None:
        try:
    ...
    def run(
        self, num_steps: int | None = None, catch_sigint: bool = True
    ) -> None:
        ...
        try:
            if catch_sigint:
                # Non-main thread: signal() raises ValueError; skip gracefully.
