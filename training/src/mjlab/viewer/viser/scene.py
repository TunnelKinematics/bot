...
def _compute_body_hull(
    ...
    if not pieces:
        return None
    merged = (
        pieces[0] if len(pieces) == 1 else trimesh.util.concatenate(pieces)
    )
    try:
...
class MjlabViserScene(ViserMujocoScene, DebugVisualizer):
    ...
    def __init__(
        ...
        self._baked_appearance_fingerprint: (
        ...
        # Ghost mesh state.
        self._ghost_handles: dict[
            tuple[int, int], viser.BatchedMeshHandle
        ] = {}
    ...
    def _create_mesh_handles_by_group(self) -> None:
        ...
        for env_idx in range(self.num_envs):
            ...
            for geom_id in range(self.mj_model.ngeom):
                ...
                body_group_geoms.setdefault((body_id, group_id), []).append(
                    geom_id
                )

            for (body_id, group_id), geom_ids in body_group_geoms.items():
                subgroups = group_geoms_by_visual_compat(
                    self.mj_model, geom_ids
                )
        ...
        with self.server.atomic():
            for variant_idx, variant in enumerate(variants.values()):
                ...
                suffix = (
                    f"/sub{variant.sub_idx}" if variant.sub_idx > 0 else ""
                )
                visible = (
                    variant.group_id < 6
                    and self.geom_groups_visible[variant.group_id]
                )
    ...
    def _compute_hull_body_meshes(self) -> None:
        ...
        for geom_id in range(self.mj_model.ngeom):
            if int(self.mj_model.geom_type[geom_id]) != int(
                mjtGeom.mjGEOM_MESH
            ):
                continue
    ...
    def _build_hull_handles(self) -> None:
            ...
            for body_id, geom_ids in body_geoms.items():
                ...
                fingerprint = tuple(
                    (
                        ...
                        tuple(
                            float(x)
                            for x in self.mj_model.geom_pos[gid].round(6)
                        ),
                        tuple(
                            float(x)
                            for x in self.mj_model.geom_quat[gid].round(6)
                        ),
                    )
                    for gid in geom_ids
                )
        ...
        for body_id, geom_ids in fixed_hull_bodies.items():
            ...
            fixed_opacities = (
                None
                if opacity >= 1.0
                else np.array([opacity], dtype=np.float32)
            )
        ...
        for variant_idx, ((body_id, _fp), v) in enumerate(variants.items()):
            ...
            self._hull_per_world_groups.append(
                _PerWorldHullGroup(
                    handle=handle, body_id=body_id, env_ids=env_ids
                )
            )
        ...
        with self.server.atomic():
            ...
            for mg in self._mesh_groups:
                if isinstance(mg, _PerWorldMeshGroup):
                    visible = (
                        mg.group_id < 6
                        and self.geom_groups_visible[mg.group_id]
                    )
                    if visible and any(
    ...
    def add_ghost_mesh(
        ...
        self._queued_ghosts.append(
            (
                qpos.copy(),
                model,
                np.asarray(mocap_pos).copy()
                if mocap_pos is not None
                else None,
                np.asarray(mocap_quat).copy()
    ...
    def add_cylinder(
        ...
        del label
        start, end = _to_numpy(start), _to_numpy(end)
        self._queued_cylinders.append(
            (start.copy(), end.copy(), radius, color)
        )
    ...
    def add_ellipsoid(
        ...
        self._queued_ellipsoids.append(
            (
                ...
                np.asarray(_to_numpy(mat), dtype=np.float32)
                .reshape(3, 3)
                .copy(),
                color,
            )
    ...
    def _sync_cylinders(self) -> None:
        ...
        colors = np.zeros((n, 3), dtype=np.uint8)
        opacity = 1.0
        for i, (start, end, radius, color) in enumerate(
            self._queued_cylinders
        ):
            s = start + self._scene_offset
            e = end + self._scene_offset
            d = e - s
    ...
    def _sync_ellipsoids(self) -> None:
        ...
        colors = np.zeros((n, 3), dtype=np.uint8)
        opacity = 1.0
        for i, (center, size, mat, color) in enumerate(
            self._queued_ellipsoids
        ):
            positions[i] = center + self._scene_offset
    ...
    def _sync_ghosts(self) -> None:
        ...
        for (
            ...
            for gi in range(model.ngeom):
                ...
                if (
                    model.body_dofnum[bid] == 0
                    and model.body_parentid[bid] == 0
                ):
                    continue
            ...
            for bid, bid_geom_ids in body_geoms.items():
                ...
                body_data.setdefault(key, []).append(
                    (
                        ...
                        (model.geom_rgba[bid_geom_ids[0]][:3] * 255).astype(
                            np.uint8
                        ),
                    )
                )
