...
class ViserPlayViewer(BaseViewer):
            ...
            with tabs.add_tab("Checkpoints", icon=viser.Icon.DATABASE):
                ...
                def _(event) -> None:
                    if event.target.value == "Sync":
                        self._actions.append(
                            (ViewerAction.FETCH_CHECKPOINT, "refresh")
                        )
                    else:
                        self._actions.append(
        ...
        if payload in ("refresh", "latest"):
            ...
            cur = next(
                (
                    lbl
                    for lbl in labels
                    if lbl.startswith(self._ckpt_mgr.current_name)
                ),
    ...
    @staticmethod
    def _should_update_cameras(
        paused: bool, has_pending_updates: bool
    ) -> bool:
    ...
    def _should_submit_scene_update(
        ...
        if counter % 2 != 0:
            return False
        return not (paused and not has_pending_updates)
