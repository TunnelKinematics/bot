...
class PlotCfg:
    ...
    min_rows_per_col: int = (
        4  # Minimum rows for height calc (prevents stretching).
    )
...
class NativeMujocoViewer(BaseViewer):
    ...
    def setup(self) -> None:
        ...
        if self.enable_perturbations:
            self.log(
                "[INFO] Interactive perturbations enabled", VerbosityLevel.INFO
            )
    ...
    def sync_env_to_viewer(self) -> None:
        ...
        assert (
            self.mjm is not None
            and self.mjd is not None
            and self.vopt is not None
        )
    ...
    def _update_reward_figures(self, viewer: mujoco.viewer.Handle) -> None:
        ...
        viewport_figs = [
            ...
            for i in range(
                min(
                    len(viewports),
                    len(self._term_names),
                    self._plot_cfg.max_viewports,
                )
            )
        ]
    ...
    def _sync_env_state_to_mjdata(
        ...
        if self.mjm.nmocap > 0:
            target_data.mocap_pos[:] = (
                sim_data.mocap_pos[env_idx].cpu().numpy()
            )
            target_data.mocap_quat[:] = (
                sim_data.mocap_quat[env_idx].cpu().numpy()
            )
        target_data.xfrc_applied[:] = (
            sim_data.xfrc_applied[env_idx].cpu().numpy()
        )
    ...
    def _stabilize_tracking_camera(self) -> None:
        ...
        if not (
            self.viewer
            and self.viewer.cam.type
            == mujoco.mjtCamera.mjCAMERA_TRACKING.value
        ):
            return
    ...
    def sync_viewer_to_env(self) -> None:
        ...
        if pert.active != 0 and pert.select > 0:
            ...
            mujoco.mj_applyFT(
                self.mjm, self.mjd, force, torque, point, body_id, qfrc
            )
    ...
    def _safe_key_callback(self, key: int) -> None:
        ...
        if self.user_key_callback:
            try:
                ...
                self.log(
                    f"[WARN] user key_callback raised: {e}",
                    VerbosityLevel.INFO,
                )
    ...
    def _handle_custom_action(
        ...
        if action == ViewerAction.PREV_ENV and self.env.unwrapped.num_envs > 1:
            ...
            self._clear_histories()
            self.log(
                f"[INFO] Switched to environment {self.env_idx}",
                VerbosityLevel.INFO,
            )
            return True
    ...
    def _setup_camera(self) -> None:
        ...
        self.viewer.cam.lookat = getattr(
            self.cfg, "lookat", self.viewer.cam.lookat
        )
...
def compute_viewports(
    ...
    for idx in range(min(num_plots, cfg.max_viewports)):
        ...
        vps.append(
            mujoco.MjrRect(left=left, bottom=bottom, width=vp_w, height=vp_h)
        )
    return vps
