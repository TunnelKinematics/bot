  ...
  to reduce geom count."""
    ...
    def function(
        ...
        if self.size[0] != self.size[1]:
            raise ValueError(
                f"The terrain must be square. Received size: {self.size}."
            )
        ...
        num_boxes_x = int(
            (self.size[0] - 2 * self.border_width) / self.grid_width
        )
        num_boxes_y = int(
            (self.size[1] - 2 * self.border_width) / self.grid_width
        )

        terrain_height = 1.0
        border_width = (
            self.size[0] - min(num_boxes_x, num_boxes_y) * self.grid_width
        )
        ...
        border_specs = [
            ...
            (
                (
                    half_border,
                    (self.size[1] - 2 * border_thickness) / 2,
                    half_terrain,
                ),
    ...
    def _create_merged_boxes(
        ...
        for i in range(num_boxes_x):
            for j in range(num_boxes_y):
                ...
                if grid_height > 0:
                    normalized_height = (height + grid_height) / (
                        2 * grid_height
                    )
                else:
    ...
    def _create_individual_boxes(
        ...
        for i in range(num_boxes_x):
            ...
            if self.holes and not (
                platform_min <= box_center_x <= platform_max
            ):
                in_y_strip = False
            else:
                in_y_strip = True
            ...
            for j in range(num_boxes_y):
                ...
                # color and avoid dividing by zero.
                if grid_height > 0:
                    normalized_height = (height_noise + grid_height) / (
                        2 * grid_height
                    )
                else:
                    normalized_height = 0.5
...
class BoxRandomSpreadTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        for _ in range(num_boxes):
            ...
            # Avoid platform.
            if (
                platform_min - size_x / 2 <= pos_x <= platform_max + size_x / 2
            ) and (
...
class BoxOpenStairsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        if self.inverted:
            platform_h_center = (
                -num_steps * step_height - 0.5 * self.step_thickness
            )
        else:
            platform_h_center = (
                num_steps + 1
...
class BoxRandomStairsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        for k in range(num_steps):
            ...
            box = body.add_geom(
                ...
                size=(
                    np.maximum(1e-6, self.step_width / 2.0),
                    np.maximum(
                        1e-6, (box_size[1] - 2 * self.step_width) / 2.0
                    ),
                    np.maximum(1e-6, total_h / 2.0),
                ),
                pos=box_pos,
        ...
        platform_size = (
            np.maximum(
                1e-6, terrain_size[0] - 2 * num_steps * self.step_width
            ),
            np.maximum(
                1e-6, terrain_size[1] - 2 * num_steps * self.step_width
            ),
        )
    ...
    def function(
        ...
        if self.border_width > 0.0:
            ...
            for b_geom in border_boxes:
                geometries.append(
                    TerrainGeometry(geom=b_geom, color=border_rgba)
                )
        ...
        floor_geom = body.add_geom(
            ...
            pos=(
                self.size[0] / 2,
                self.size[1] / 2,
                -self.floor_depth - floor_h / 2,
            ),
        )
        geometries.append(
            TerrainGeometry(geom=floor_geom, color=(0.1, 0.1, 0.1, 1.0))
        )
        ...
        geometries.append(
            TerrainGeometry(
                geom=platform_geom, color=brand_ramp(_MUJOCO_GREEN, 0.5)
            )
        )
        ...
        origin = np.array(
            [self.size[0] / 2, self.size[1] / 2, self.stone_height]
        )
        return TerrainOutput(origin=origin, geometries=geometries)
...
class BoxNarrowBeamsTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        spacing_sin = (
            float(np.sin(np.pi / num_beams)) if num_beams > 1 else 0.0
        )
        if spacing_sin > 1e-9:
            max_no_protrude = beam_width / (2.0 * np.sqrt(2.0) * spacing_sin)
            platform_radius = float(
                min(self.platform_width / 2.0, max_no_protrude)
            )
        else:
        ...
        geometries.append(
            TerrainGeometry(
                geom=platform_geom, color=brand_ramp(_MUJOCO_BLUE, 0.5)
            )
        )
        ...
        for angle in angles:
            ...
            extra_length = (
                (beam_width / 2) * min(cos_a, sin_a) / max(cos_a, sin_a)
            )
        ...
        origin = np.array(
            [self.size[0] / 2, self.size[1] / 2, self.beam_height]
        )
        return TerrainOutput(origin=origin, geometries=geometries)
        ...
        for _ in range(self.num_rings):
            ...
            for dy in [-1, 1]:
                box_pos = (
                    terrain_center[0],
                    terrain_center[1]
                    + dy * (ring_outer_size[1] - ring_width) / 2,
                    z_pos,
                )
                box = body.add_geom(
                    type=mujoco.mjtGeom.mjGEOM_BOX,
            ...
            for dx in [-1, 1]:
                box_pos = (
                    terrain_center[0]
                    + dx * (ring_outer_size[0] - ring_width) / 2,
                    terrain_center[1],
                    z_pos,
                )
                box = body.add_geom(
                    ...
                    size=(
                        np.maximum(1e-6, ring_width / 2.0),
                        np.maximum(
                            1e-6, (ring_outer_size[1] - 2 * ring_width) / 2.0
                        ),
                        half_h,
                    ),
                    pos=box_pos,
