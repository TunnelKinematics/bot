...
def _fractal_perlin_noise_2d(
    ...
    for _ in range(octaves):
        noise += amplitude * perlin(
            xx * frequency / x_size, yy * frequency / y_size, p
        )
...
def _compute_flat_patches(
    ...
    if flat_patch_sampling is None:
        return None
    physical_heights = (
        noise.astype(np.float64) - noise.min()
    ) * vertical_scale
  ...
  horizontal_scale if non-zero."""
    ...
    def function(
        ...
        if border_pixels > 0:
            ...
            platform_width = int(
                self.platform_width / self.horizontal_scale / 2
            )
        ...
        hfield_z_offset = -max_physical_height if self.inverted else 0

        physical_heights = (
            hfield_z_offset + normalized_elevation * max_physical_height
        )
        material_name = color_by_height(
            spec, noise, unique_id, physical_heights
        )
  ...
  a curriculum."""
    ...
    def function(
        ...
        if border_pixels > 0:
            ...
            height_range = np.arange(
                height_min, height_max + height_step, height_step
            )
...
class HfWaveTerrainCfg(SubTerrainCfg):
  ...
  horizontal_scale if non-zero."""
    ...
    def function(
        ...
        if self.num_waves <= 0:
            raise ValueError(
                f"Number of waves must be positive. Got: {self.num_waves}"
            )
...
class HfDiscreteObstaclesTerrainCfg(SubTerrainCfg):
  ...
  """
    ...
    def function(
        ...
        obs_width_min = int(
            self.obstacle_width_range[0] / self.horizontal_scale
        )
        obs_width_max = int(
            self.obstacle_width_range[1] / self.horizontal_scale
        )
        ...
        noise = np.zeros(
            (inner_width_pixels, inner_length_pixels), dtype=np.int16
        )
        ...
        for _ in range(self.num_obstacles):
            if self.obstacle_height_mode == "choice":
                h = rng.choice(
                    np.array([-obs_h, -obs_h // 2, obs_h // 2, obs_h])
                )
            else:
                h = obs_h
            ...
            obs_len = (
                w if self.square_obstacles else rng.choice(obs_width_range)
            )
        ...
        if border_pixels > 0:
            outer_noise = np.zeros(
                (width_pixels, length_pixels), dtype=np.int16
            )
            outer_noise[
...
class HfPerlinNoiseTerrainCfg(SubTerrainCfg):
    ...
    def function(
        ...
        effective_scale = self.scale * (
            self.resolution / self.horizontal_scale
        )
        ...
        if border_pixels > 0:
            ...
            noise_range = (
                noise_max - noise_min if noise_max > noise_min else 1.0
            )
            ...
            normalized_elevation = (
                (noise_raw - noise_min) / noise_range
            ).astype(np.float32)
