...
class EventManager(ManagerBase):
    ...
    def __init__(self, cfg: dict[str, EventTermCfg], env: ManagerBasedRlEnv):
        ...
        self._mode_class_term_cfgs: dict[EventMode, list[EventTermCfg]] = (
            dict()
        )
    ...
    def get_term_cfg(self, term_name: str) -> EventTermCfg:
        ...
        raise ValueError(
            f"Event term '{term_name}' not found in active terms."
        )
    ...
    def reset(self, env_ids: torch.Tensor | None = None):
        ...
        if "interval" in self._mode_term_cfgs:
            for index, term_cfg in enumerate(
                self._mode_class_term_cfgs["interval"]
            ):
                if not term_cfg.is_global_time:
                    ...
                    self._interval_term_time_left[index][env_ids] = (
                        sampled_interval
                    )
        return {}
    ...
    def apply(
        ...
        for index, term_cfg in enumerate(self._mode_term_cfgs[mode]):
            ...
            if mode == "interval":
                ...
                if term_cfg.is_global_time:
                    if time_left < 1e-6:
                        ...
                        lower, upper = term_cfg.interval_range_s
                        sampled_interval = (
                            torch.rand(1) * (upper - lower) + lower
                        )
                    ...
                    if len(valid_env_ids) > 0:
                        ...
                        term_cfg.func(
                            self._env, valid_env_ids, **term_cfg.params
                        )
                        fired = True
            elif mode == "step":
                term_cfg.func(self._env, None, **term_cfg.params)
                ...
                assert env_ids is not None, (
                    "reset events require concrete env_ids, got None"
                )
                ...
                if min_step_count == 0:
                    ...
                    triggered_at_least_once = (
                        self._reset_term_last_triggered_once[index][env_ids]
                    )
                    steps_since_triggered = (
                        global_env_step_count - last_triggered_step
                    )
            ...
            if fired:
                level = getattr(
                    term_cfg.func, "recompute", RecomputeLevel.none
                )
