...
class ObservationManager(ManagerBase):
        ...
        for group_name, group_dim in self._group_obs_dim.items():
            ...
            for index, (name, dims, term_cfg) in enumerate(obs_terms):
                if (
                    ...
                    and term_cfg.flatten_history_dim
                ):
                    # Flattened history: show (9,) ← 3×(3,)
                    original_size = (
                        int(np.prod(dims)) // term_cfg.history_length
                    )
                    original_shape = (
                        (original_size,) if len(dims) == 1 else dims[1:]
                    )
                    shape_str = (
                        f"{dims}  ← {term_cfg.history_length}×{original_shape}"
                    )
                else:
    ...
    def get_term_cfg(
        ...
        if group_name not in self._group_obs_term_names:
            raise ValueError(
                f"Group '{group_name}' not found in active groups."
            )
            ...
            for term_name in self._group_obs_term_names[group_name]:
                ...
                if term_name in self._group_obs_term_delay_buffer[group_name]:
                    self._group_obs_term_delay_buffer[group_name][
                        term_name
                    ].reset(batch_ids=batch_ids)
                if (
                    term_name
                    in self._group_obs_term_history_buffer[group_name]
                ):
    ...
    def _check_and_handle_nans(
        ...
        if policy == "error":
            nan_mask = torch.isnan(tensor).any(dim=-1) | torch.isinf(
                tensor
            ).any(dim=-1)
    ...
    def compute_group(
        ...
        for term_name, term_cfg in obs_terms:
            obs: torch.Tensor = term_cfg.func(
                self._env, **term_cfg.params
            ).clone()
            ...
            if term_cfg.delay_max_lag > 0:
                delay_buffer = self._group_obs_term_delay_buffer[group_name][
                    term_name
                ]
        ...
        if (
            not group_cfg.nan_check_per_term
            and group_cfg.nan_policy != "disabled"
        ):
            if self._group_obs_concatenate[group_name]:
    ...
    def _prepare_terms(self) -> None:
        ...
        self._group_obs_term_delay_buffer: dict[
            str, dict[str, DelayBuffer]
        ] = dict()
        self._group_obs_term_history_buffer: dict[
            str, dict[str, CircularBuffer]
        ] = dict()
        ...
        for group_name, group_cfg in self.cfg.items():
            ...
            for term_name, term_cfg in group_cfg.terms.items():
                ...
                if hasattr(term_cfg.func, "reset") and callable(
                    term_cfg.func.reset
                ):
                ...
                if term_cfg.noise is not None and isinstance(
                    ...
                    assert issubclass(
                        noise_model_cls, noise_model.NoiseModel
                    ), (
            ...
            self._group_obs_term_delay_buffer[group_name] = (
                group_entry_delay_buffer
            )
            self._group_obs_term_history_buffer[group_name] = (
                group_entry_history_buffer
            )
