...
_FIELD_CONFIGS = [
    _FieldConfig(
        "joint_names", "joint_ids", "find_joints", "num_joints", "joint"
    ),
    _FieldConfig(
        "body_names", "body_ids", "find_bodies", "num_bodies", "body"
    ),
...
class SceneEntityCfg:
    ...
    actuator_ids: list[int] | slice = field(
    ...
    material_ids: list[int] | slice = field(
        default_factory=lambda: slice(None)
    )
    ...
    def _resolve_field(self, entity: Entity, config: _FieldConfig) -> None:
        ...
        if names is not None and isinstance(ids, list):
            ...
            self._resolve_ids_to_names(
                ids, entity_all_names, config.names_attr
            )

    def _normalize_to_list(
        self, value: str | int | tuple | list | None
    ) -> list | None:
    ...
    def _resolve_names_to_ids(
        ...
        found_ids, found_names = find_method(
        ...
        if len(found_ids) == entity_count and found_names == list(
            entity_all_names
        ):
