...
class TerminationManager(ManagerBase):
    ...
    def __init__(
        self, cfg: dict[str, TerminationTermCfg], env: ManagerBasedRlEnv
    ):
    ...
    def compute(self) -> torch.Tensor:
        ...
        for name, term_cfg in zip(
            self._term_names, self._term_cfgs, strict=False
        ):
    ...
    def get_active_iterable_terms(
        ...
        for key in self._term_dones:
            terms.append(
                (key, [self._term_dones[key][env_idx].float().cpu().item()])
            )
        return terms
