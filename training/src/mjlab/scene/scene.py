...
class Scene:
    ...
    def __getitem__(self, key: str) -> Any:
        ...
        raise KeyError(
            f"Scene element '{key}' not found. Available: {available}"
        )
    ...
    def initialize(
        ...
        ctx_sensors = [
            s for s in self._sensors.values() if s.requires_sensor_context
        ]
        if ctx_sensors:
            camera_sensors = [
                s for s in ctx_sensors if isinstance(s, CameraSensor)
            ]
            raycast_sensors = [
                s for s in ctx_sensors if isinstance(s, RayCastSensor)
            ]
