...
def make_gait_golden(env: ManagerBasedRlEnv, gait: dict) -> dict:
    ...
    commands = np.concatenate(
        [
            ...
            rng.uniform(
                [-1.0, -0.5, -2.0], [1.0, 0.5, 2.0], size=(_GOLDEN_N_RANDOM, 3)
            ),
        ]
    )
    n = len(commands)
    ...
    if gait_frame_kind(env) in ("mixed", "mixed_jump"):
        from mjlab.tasks.pupper_gait.mdp.mixed_gaits import (
...
def make_jump_golden(env: ManagerBasedRlEnv, jump: dict) -> dict:
    ...
    residual = float(np.abs(deploy - train).max())
    print(
        f"[INFO]: deploy-vs-training jump reference residual = {residual:.2e}"
    )
    if residual >= _GOLDEN_TOL:
        raise RuntimeError(
    ...
    t_all = np.concatenate(
        [t, np.asarray(_JUMP_GOLDEN_EXTRA_T, dtype=np.float64)]
    )
...
def make_slot_golden(
    env: ManagerBasedRlEnv, gait: dict, jump_slot: dict
) -> list:
    """Jump-slot composite cases, cross-checked against the training term."""
    import mjlab.tasks.pupper_gait.mdp.mixed_jump as mixed_jump_mod
    from mjlab.tasks.pupper_gait.mdp.mixed_jump import (
    ...
    combos = [
        (t_in, cmd) for t_in in _SLOT_CASE_T_IN for cmd in _SLOT_CASE_CMDS
    ]
    n = len(combos)
    ...
    residual = float(np.abs(deploy - train).max())
    print(
        f"[INFO]: deploy-vs-training slot composite residual = {residual:.2e}"
    )
    if residual >= _GOLDEN_TOL:
        raise RuntimeError(
...
def run_export(task_id: str, cfg: ExportConfig) -> Path:
    ...
    env_cfg.scene.num_envs = (
        len(_GOLDEN_CASES) + _GOLDEN_N_RANDOM
    ...
    if cfg.checkpoint_file is not None:
        ...
        log_root_path = (
            Path(cfg.log_root) / agent_cfg.experiment_name
        ).resolve()
    ...
    if err >= cfg.parity_tol:
        raise RuntimeError(
            f"Export parity FAILED ({err:.2e}) -- do not deploy."
        )
