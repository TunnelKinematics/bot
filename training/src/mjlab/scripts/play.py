    ...
    if is_tracking_task:
        ...
        if cfg.motion_file is not None and Path(cfg.motion_file).exists():
            ...
            if cfg.motion_file is not None:
                ...
                api = wandb.Api()
                if (
                    cfg.wandb_run_path is None
                    and cfg.checkpoint_file is not None
                ):
                    raise ValueError(
                ...
                if cfg.wandb_run_path is not None:
                    ...
                    art = next(
                        (
                            a
                            for a in wandb_run.used_artifacts()
                            if a.type == "motions"
                        ),
                        None,
                    )
                    if art is None:
                        raise RuntimeError(
                            "No motion artifact found in the run."
                        )
                    motion_cmd.motion_file = str(
        ...
        if cfg.agent == "zero":
            ...
            class PolicyZero:
                def __call__(self, obs) -> torch.Tensor:
                    del obs
                    return torch.zeros(
                        action_shape, device=env.unwrapped.device
                    )
            ...
            class PolicyRandom:
                def __call__(self, obs) -> torch.Tensor:
                    ...
                    return (
                        2
                        * torch.rand(action_shape, device=env.unwrapped.device)
                        - 1
                    )
            ...
            def fetch_available_wandb() -> list[tuple[str, str]]:
                ...
                for f in wandb_run.files():
                    ...
                    ago = format_time_ago(
                        int(
                            (
                                now - _parse_wandb_dt(f.updated_at)
                            ).total_seconds()
                        )
                    )
                    entries.append((f.name, ago, step))
