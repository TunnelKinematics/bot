...
def _randomize_model_field(
    ...
    *,
    ...
    if (
        isinstance(ranges, dict)
        and ranges
        and isinstance(next(iter(ranges)), str)
    ):
    ...
    narrowed_ranges: tuple[float, float] | dict[int, tuple[float, float]] = (
        ranges  # type: ignore[assignment]
    )
    ...
    if operation.uses_defaults:
        base_values = _select_default_values(
...
def _determine_target_axes(
    ...
    if axes is not None:
        ...
        isinstance(ranges, dict)
        and ranges
        and isinstance(next(iter(ranges)), int)
    ):
...
def _generate_random_values(
    ...
    for axis in target_axes:
        ...
        random_vals = distribution.sample(
            lower_bound, upper_bound, shape, device
        )
...
def _randomize_categorical_field(
    ...
    *,
    ...
    entity_indices = _get_entity_indices(
        asset.indexing, asset_cfg, entity_type, False
    )
    env_grid, entity_grid = torch.meshgrid(
        env_ids, entity_indices, indexing="ij"
    )
