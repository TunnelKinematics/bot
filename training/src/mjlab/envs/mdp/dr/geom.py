...
def _recompute_geom_bounds(
    ...
    entity_indices = _get_entity_indices(
        asset.indexing, asset_cfg, "geom", False
    )
    ...
    rbound = torch.where(
        is_ellipsoid, torch.maximum(s0, torch.maximum(s1, s2)), rbound
    )
    rbound = torch.where(is_cylinder, torch.sqrt(s0 * s0 + s1 * s1), rbound)
    rbound = torch.where(
        is_box, torch.sqrt(s0 * s0 + s1 * s1 + s2 * s2), rbound
    )
