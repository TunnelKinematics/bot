    ...
    for actuator in actuators:
        ...
        if isinstance(actuator, BuiltinPositionActuator) or (
            ...
            if op.name == "scale":
                default_gainprm = env.sim.get_default_field("actuator_gainprm")
                env.sim.model.actuator_gainprm[
                    env_ids[:, None], ctrl_ids, 4
                ] = default_gainprm[ctrl_ids, 4] * kp_samples
                env.sim.model.actuator_gainprm[
                    env_ids[:, None], ctrl_ids, 6
...
def effort_limits(
    ...
    for actuator in actuators:
        ...
        if isinstance(
            actuator, (BuiltinPositionActuator, BuiltinDcMotorActuator)
        ) or (
            isinstance(actuator, XmlActuator)
            and actuator.command_field == "position"
        ):
            if op.name == "scale":
                default_forcerange = env.sim.get_default_field(
                    "actuator_forcerange"
                )
            ...
            if actuator.transmission_type == TransmissionType.JOINT:
                ...
                target_global_ids = asset.indexing.joint_ids[
                    actuator.target_ids
                ]
            else:
                field = "tendon_actfrcrange"
                target_global_ids = asset.indexing.tendon_ids[
                    actuator.target_ids
                ]
