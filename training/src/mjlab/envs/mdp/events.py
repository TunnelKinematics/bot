...
def reset_root_state_uniform(
    ...
    # Velocities.
    vel_samples = _sample_se3_range(
        velocity_range, (len(env_ids), 6), env.device
    )
...
def reset_root_state_from_flat_patches(
    ...
    patch_ids = torch.randint(
        0, num_patches, (len(env_ids),), device=env.device
    )
    ...
    **Application point.** By default, forces act at each body's center of mass.
    ...
    def __call__(
        ...
        if body_point_offset is not None:
            ...
            body_quat = self._asset.data.body_com_quat_w[trigger_ids][
                :, self._body_ids
            ]
            # Rotate offset into world frame: (n, num_bodies, 3).
            offset_w = quat_apply(
                body_quat.reshape(-1, 4),
                offset_local.expand(n * self._num_bodies, 3),
    ...
    def debug_vis(self, visualizer: DebugVisualizer) -> None:
        ...
        com_quat = (
            self._asset.data.body_com_quat_w if offset is not None else None
        )
