...
class DifferentialIKAction(ActionTerm):
    ...
    def __init__(self, cfg: DifferentialIKActionCfg, env: ManagerBasedRlEnv):
        ...
        self._joint_dof_ids = self._entity.indexing.joint_v_adr[
            self._joint_ids
        ]
        ...
        q_target = self._entity.data.default_joint_pos[
            :, self._joint_ids
        ].clone()
    ...
    def process_actions(self, actions: torch.Tensor) -> None:
        ...
        if self._action_dim == 3:
            if self.cfg.use_relative_mode:
                self._desired_pos[:] = (
                    frame_pos + actions * self.cfg.delta_pos_scale
                )
            else:
    ...
    def compute_dq(self) -> torch.Tensor:
        ...
        JTJ = wp2 * torch.einsum(
            "bti,btj->bij", jacp, jacp
        ) + wo2 * torch.einsum("bti,btj->bij", jacr, jacr)
    ...
    def apply_actions(self) -> None:
        ...
        self._entity.set_joint_position_target(
            q_target, joint_ids=self._joint_ids
        )
    ...
    def _resolve_frame(self, frame_name: str) -> None:
        ...
        if self._frame_type == "body":
            ...
            local_id = ids[0]
            self._frame_id = int(
                self._entity.indexing.body_ids[local_id].item()
            )
            ...
            local_id = ids[0]
            self._frame_id = int(
                self._entity.indexing.site_ids[local_id].item()
            )
            self._body_id = int(
                self._env.sim.mj_model.site_bodyid[self._frame_id]
            )
        elif self._frame_type == "geom":
            ids, _ = self._entity.find_geoms(frame_name)
            local_id = ids[0]
            self._frame_id = int(
                self._entity.indexing.geom_ids[local_id].item()
            )
            self._body_id = int(
                self._env.sim.mj_model.geom_bodyid[self._frame_id]
            )
        else:
