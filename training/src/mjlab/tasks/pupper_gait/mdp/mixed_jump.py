...
from mjlab.tasks.pupper.mdp.observations import (
    PupperObsNoise,
    pupper_actor_obs,
)
...
def _slot_starts(env: ManagerBasedRlEnv) -> torch.Tensor:
    ...
    if buf is None:
        buf = torch.full(
            (env.num_envs, _MAX_JUMPS), float("inf"), device=env.device
        )
...
def reset_jump_schedule(
    ...
    count = torch.where(
        ...
        torch.zeros_like(r),
        1.0
        + torch.floor(3.0 * (r - p_no_jump) / (1.0 - p_no_jump)).clamp(
            max=2.0
        ),
    )
    ...
    t1 = _quantize(
        torch.empty(n, device=env.device).uniform_(
            min_start_s, max(hi, min_start_s + 1e-3)
        )
    )
    prev = t1
    cols = [t1]
...
class MixedJumpVelocityCommand(PupperVelocityCommand):
    ...
    def create_gui(
        ...
        super().create_gui(
            name, server, get_env_idx, on_change, request_action
        )
        ...
        def _(_) -> None:
            ...
            t = (
                float(self._env.episode_length_buf[get_env_idx()])
                * self._env.step_dt
            )
            print(
                f"[viewer] jump scheduled at t={start:.2f}s (in {start - t:.2f}s)"
            )
...
def mixed_jump_reference_offset(
    ...
    jump_ref = _lookup(
        _reference_table(env, n_samples, "jump"), phase, n_samples
    )
    fade = torch.clamp(
...
class mixed_jump_tracking:
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        ...
        def _resolve(spec) -> torch.Tensor:
            _, _, vals = resolve_matching_names_values(
                data=spec, list_of_strings=joint_names
            )
    ...
    def __call__(self, env: ManagerBasedRlEnv, **kwargs) -> torch.Tensor:
        ...
        env.episode_length_buf = (saved.float() - self.phase_lead_steps).to(
            saved.dtype
        )
        try:
...
def slot_jump_airborne_height(
    ...
    height = torch.clamp(
        asset.data.root_link_pos_w[:, 2] - stand_height,
        min=0.0,
        max=height_cap,
    )
...
def stand_still_pose_outside_slot(
...
def stand_still_joint_velocity_outside_slot(
    ...
    return _outside_gate(env, slot_s) * stand_still_joint_velocity(
        env, **kwargs
    )
