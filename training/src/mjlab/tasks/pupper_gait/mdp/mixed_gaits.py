...
def mixed_reference_offset(
    ...
    trot = _lookup(
        _reference_table(env, n_samples, "trot"), eff_phase, n_samples
    )
    ...
    reach = _lookup(
        _reference_table(env, n_samples, "reach"), eff_phase, n_samples
    )
    reach_back = _lookup(
        _reference_table(env, n_samples, "reach_back"), eff_phase, n_samples
    )
    lift = _lookup(
        _reference_table(env, n_samples, "lift"), eff_phase, n_samples
    )
...
class mixed_gait_tracking:
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        ...
        for gait in ("trot", "reach", "reach_back", "lift"):
            _reference_table(
                env, int(self._p["n_samples"]), gait
            )  # warm the caches
    ...
    def __call__(self, env: ManagerBasedRlEnv, **kwargs) -> torch.Tensor:
        ...
        error_squared = torch.square(
            asset.data.joint_pos[:, self.ids] - reference
        )
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        ...
        for right, left in pairs:
            for name in (right, left):
                ...
                assert jname == [name], (
                    f"roll (motor 2) joint {name} not found"
                )
                ids.append(jid[0])
