...
_JUMP_PHASE_START = (
    0.0 if gait_reference_module._CAPTURED_JUMP_FILE.exists() else 0.2
)
...
class jump_tracking:
    ...
    def __init__(self, cfg: RewardTermCfg, env: ManagerBasedRlEnv):
        ...
        self.loose_until_s = float(
            cfg.params.get("loose_until_s", float("inf"))
        )
...
class torso_clearance:
    ...
    def __call__(
        ...
        bottom = (
            asset.data.root_link_pos_w[:, 2] + world[:, :, 2].min(dim=1).values
        )
        return torch.clamp((min_clearance - bottom) / min_clearance, min=0.0)


def _post_launch_gate(
    env: ManagerBasedRlEnv, gate_before_s: float
) -> torch.Tensor:
