...
def gait_reference_metadata(env) -> dict[str, Any]:
    ...
    if kind == "zeroref":
        ...
        fast = (
            _reference_table(env, n_samples, fast_gait).detach().cpu().numpy()
        )
    ...
    if kind in ("mixed", "mixed_jump"):
        ...
        back = (
            _reference_table(env, n_samples, "reach_back")
            .detach()
            .cpu()
            .numpy()
        )
...
def reference_offset_numpy(
    ...
    lift = (
        np.asarray(gait["lift_table"], dtype=np.float64)
...
def mixed_jump_reference_offset_numpy(
    ...
    fade = np.clip(
        np.minimum(
            np.where(in_window, t_in, 0.0),
            active_s - np.where(in_window, t_in, 0.0),
        )
        / fade_s,
        0.0,
        1.0,
...
def gait_reference_offset_torch(
    ...
    gallop = torch.tensor(
        gait["gallop_table"], dtype=torch.float32, device=device
    )
