...
class pupper_variable_posture:
    ...
    def __call__(
        ...
        turning = (
            ~standing
            & (linear_speed < turn_linear_max)
            & (angular_speed > turn_angular_min)
        )
...
def stand_still_pose(
    ...
    dev = torch.sum(
        torch.abs(asset.data.joint_pos[:, ids] - default_joint_pos[:, ids]),
        dim=1,
    )
...
def forward_speed(
    ...
    return torch.clamp(
        direction * asset.data.root_link_lin_vel_b[:, 0], 0.0, clip
    )
