...
def get_imu_latency_buffer(
    ...
    if buffer is None or buffer.max_lag != len(distribution) - 1:
        buffer = LatencyBuffer(
            distribution, env.num_envs, (_IMU_DIM,), env.device
        )
...
def pupper_actor_obs(
    ...
    if add_noise:
        ...
        last_action = last_action + _uniform(
            last_action.shape, noise.last_action, device
        )

    # IMU latency, post-noise, on the 6 IMU dims only (mirrors the MJX env).
    if (
        imu_latency_distribution is not None
        and len(imu_latency_distribution) > 1
    ):
