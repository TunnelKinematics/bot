...
class PupperVelocityCommand(UniformVelocityCommand):
    ...
    def __init__(self, cfg: PupperVelocityCommandCfg, env: ManagerBasedRlEnv):
        ...
        self.heading_hold_target = torch.zeros(
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        ...
        self._capture_heading[env_ids] = (
            True  # recapture heading on the next update
        )
        r = torch.empty(len(env_ids), device=self.device)
        self.is_turn_env[env_ids] = (
            r.uniform_(0.0, 1.0) <= self.cfg.rel_turn_envs
        )
        ...
        if len(turn_ids) > 0:
            ...
            self.vel_command_b[turn_ids, 2] = sign * yaw.abs().clamp(
                min=self.cfg.turn_min
            )
