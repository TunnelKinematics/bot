...
def _frame_reversal_index(
    history_length: int, single_obs_dim: int
) -> np.ndarray:
...
def convert_pupper_policy(
    ...
    *,
    ...
    if command_clip is not None:
        ...
        if unknown:
            raise ValueError(
                f"command_clip has unknown axes: {sorted(unknown)}"
            )
        for axis, (lo, hi) in command_clip.items():
            if not lo <= hi:
                raise ValueError(
                    f"command_clip.{axis} is inverted: ({lo}, {hi})"
                )
        ...
        layers.append(
            {
                ...
                "activation": ""
                if is_final
                else "elu",  # "" = linear (RTNeural).
                "shape": [None, int(w.shape[0])],
                "weights": [
                    w.T.tolist(),
                    b.tolist(),
                ],  # RTNeural wants [in][out].
            }
        )
...
def export_pupper_policy_from_env(
    ...
    *,
    ...
    default_np = (
        robot.data.default_joint_pos[0].detach().cpu().numpy().tolist()
    )
...
def json_forward(policy: dict[str, Any], x: np.ndarray) -> np.ndarray:
    ...
    for layer in policy["layers"]:
        ...
        if act == "elu":
            ...
            raise ValueError(
                f"Unsupported activation in parity forward: {act!r}"
            )
    return a


def to_robot_order(
    x_mjlab: torch.Tensor | np.ndarray,
    history_length: int,
    single_obs_dim: int,
):
