...
_ABDUCTION_JOINTS = (
    r"leg_.*_2",
)  # Mirrors the notebook's [1::3] abduction slice.
...
def pupper_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    for sensor in cfg.scene.sensors or ():
        ...
        if sensor.name == "foot_height_scan":
            ...
            sensor.frame = tuple(
                ...
                for s in FOOT_SITE_NAMES
            )

    feet_ground_cfg = ContactSensorCfg(
        name="feet_ground_contact",
        primary=ContactMatch(
            mode="subtree", pattern=_FOOT_BODIES, entity="robot"
        ),
    ...
    self_collision_cfgs = tuple(
        ContactSensorCfg(
            ...
            primary=ContactMatch(
                mode="geom",
                pattern=rf"^leg_front_{side}_3_collision$",
                entity="robot",
            ),
            secondary=ContactMatch(
                mode="geom",
                pattern=rf"^leg_back_{side}_3_collision$",
                entity="robot",
            ),
            fields=("found", "force"),
            reduce="netforce",
    ...
    cfg.scene.sensors = (
        ...
        + (feet_ground_cfg, knee_ground_cfg)
        + self_collision_cfgs
    )
    ...
    if PUPPER_LATENCY_MODEL == "mjx":
        ...
        assert entity.articulation is not None, (
            "Pupper robot cfg must be articulated"
        )
    ...
    if PUPPER_LATENCY_MODEL == "mjx":
        critic.terms["applied_action"] = ObservationTermCfg(
...
def _set_pupper_rewards(cfg: ManagerBasedRlEnvCfg) -> None:
    ...
    if "pose" in r:
        ...
        r["pose"].params["std_walking"] = {
            r".*_1": 7,
            r".*_2": 0.5,
            r".*_3": 7,
        }
        r["pose"].params["std_running"] = {
            r".*_1": 10,
            r".*_2": 0.5,
            r".*_3": 10,
        }
    ...
    r["orientation_l2"] = RewardTermCfg(
    ...
    r["joint_torques_l2"] = RewardTermCfg(
    ...
    r["abduction_angle"] = RewardTermCfg(
        ...
        weight=0.0,
        params={
            "asset_cfg": SceneEntityCfg("robot", joint_names=_ABDUCTION_JOINTS)
        },
    )
    ...
    for _side in ("r", "l"):
        r[f"self_collision_{_side}"] = RewardTermCfg(
            ...
            weight=0.0,
            params={
                "sensor_name": f"self_collision_{_side}",
                "force_threshold": 1.0,
            },
        )
