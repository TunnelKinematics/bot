...
def out_of_terrain_bounds(
    ...
    half_x = (
        0.5 * (num_rows * terrain_generator.size[0])
        + terrain_generator.border_width
    )
    half_y = (
        0.5 * (num_cols * terrain_generator.size[1])
        + terrain_generator.border_width
    )
    limit_x = max(0.0, half_x - margin)
    limit_y = max(0.0, half_y - margin)

    return (root_xy_w[:, 0].abs() > limit_x) | (
        root_xy_w[:, 1].abs() > limit_y
    )
