...
class UniformVelocityCommand(CommandTerm):
    ...
    def __init__(self, cfg: UniformVelocityCommandCfg, env: ManagerBasedRlEnv):
        ...
        if self.cfg.ranges.heading and not self.cfg.heading_command:
            raise ValueError(
                "ranges.heading is set but heading_command=False."
            )
    ...
    def _resample_command(self, env_ids: torch.Tensor) -> None:
        ...
        self.is_world_env[env_ids] = (
            r.uniform_(0.0, 1.0) <= self.cfg.rel_world_envs
        )
        ...
        self.is_forward_env[env_ids] = (
            r.uniform_(0.0, 1.0) <= self.cfg.rel_forward_envs
        )
    ...
    def compute(self, dt: float) -> None:
        ...
        if self.cfg.heading_hold_kp is not None:
            ...
            active = (
                raw[:, 2].abs() < self.cfg.heading_hold_yaw_threshold
            ) & (
                torch.norm(raw[:, :2], dim=1)
                > self.cfg.heading_hold_walk_threshold
            )
    ...
    def _debug_vis_impl(self, visualizer: DebugVisualizer) -> None:
        ...
        for batch in env_indices:
            ...
            cmd_lin_to = local_to_world(
                (np.array([0, 0, z_offset]) + np.array([cmd[0], cmd[1], 0]))
                * scale
            )
            ...
            act_lin_to = local_to_world(
                (
                    ...
                    + np.array([lin_vel_b[0], lin_vel_b[1], 0])
                )
                * scale
            )
            visualizer.add_arrow(
            ...
            act_ang_to = local_to_world(
                (np.array([0, 0, z_offset]) + np.array([0, 0, ang_vel_b[2]]))
                * scale
            )
            visualizer.add_arrow(
                act_ang_from,
