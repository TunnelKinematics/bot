...
class upright:
    ...
    def debug_vis(self, visualizer: DebugVisualizer) -> None:
        ...
        terrain_normal = terrain_normal_from_sensors(
            env, self._terrain_sensor_names
        )
...
class feet_swing_height:
    ...
    def __call__(
        ...
        cost = (
            torch.sum(torch.square(error) * first_contact.float(), dim=1)
            * active
        )
...
def soft_landing(
    ...
    num_landings = torch.sum(first_contact.float())
    mean_landing_force = torch.sum(landing_impact) / torch.clamp(
        num_landings, min=1
    )
