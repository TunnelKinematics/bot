  ...
  material set to this material. ``None`` means no assignment."""
    ...
    def edit_spec(self, spec: mujoco.MjSpec) -> None:
        ...
        if self.texture is not None:
            mat.textures[mujoco.mjtTextureRole.mjTEXROLE_RGB.value] = (
                self.texture
            )
  ...
  matched meshes, or a dict mapping regex patterns to per-mesh values."""
    ...
    def validate(self) -> None:
        ...
        for pattern, value in items:
            ...
            if value <= 3:
                where = (
                    f" for pattern '{pattern}'" if pattern is not None else ""
                )
                raise ValueError(
...
class CollisionCfg(SpecCfg):
  ...
  non-negative."""
    ...
    def validate(self) -> None:
        ...
        if isinstance(self.solmix, (int, float)) and not (
            0 <= self.solmix <= 1
        ):
    ...
    def edit_spec(self, spec: mujoco.MjSpec) -> None:
        ...
        for i, geom_name in enumerate(geom_subset):
            ...
            CollisionCfg.set_array_field(
                geom.friction, resolved_fields["friction"][i]
            )
            CollisionCfg.set_array_field(
                geom.solref, resolved_fields["solref"][i]
            )
            CollisionCfg.set_array_field(
                geom.solimp, resolved_fields["solimp"][i]
            )
...
class LightCfg(SpecCfg):
    ...
    def edit_spec(self, spec: mujoco.MjSpec) -> None:
        ...
        body = spec.worldbody if self.body == "world" else spec.body(self.body)
