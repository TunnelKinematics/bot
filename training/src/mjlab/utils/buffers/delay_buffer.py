...
class DelayBuffer:
    ...
    def __init__(
        ...
        if update_period < 0:
            raise ValueError(
                f"update_period must be >= 0, got {update_period}"
            )
    ...
    def compute(self) -> torch.Tensor:
        ...
        valid_lags = torch.minimum(
            self._current_lags, self._buffer.current_length - 1
        )
    ...
    def _update_lags(self) -> None:
        ...
        self._current_lags = torch.where(
            should_update, new_lags, self._current_lags
        )
        self._step_count += 1
