...
def auto_wrap_fixed_base_mocap(
    ...
    def wrapper() -> mujoco.MjSpec:
        ...
        root_body = (
            original_spec.bodies[1] if len(original_spec.bodies) > 1 else None
        )
        ...
        keyframes = [
            (np.array(k.qpos), np.array(k.ctrl), k.name)
            for k in original_spec.keys
        ]
        ...
        mocap_body = wrapper_spec.worldbody.add_body(
            name="mocap_base", mocap=True
        )
        ...
        for qpos, ctrl, name in keyframes:
            wrapper_spec.add_key(
                name=name, qpos=qpos.tolist(), ctrl=ctrl.tolist()
            )
...
def copy_material_data(
    src: mujoco.MjsMaterial, dst: mujoco.MjsMaterial
) -> None:
