...
_DISABLE_FLAG_MAP: dict[str, int] = {
    name.removeprefix("mjDSBL_").lower(): getattr(
        mujoco.mjtDisableBit, name
    ).value
    for name in dir(mujoco.mjtDisableBit)
    if name.startswith("mjDSBL_")
}
_ENABLE_FLAG_MAP: dict[str, int] = {
    name.removeprefix("mjENBL_").lower(): getattr(
        mujoco.mjtEnableBit, name
    ).value
...
class SimulationCfg:
  ...
  constraints. If None, a heuristic value is used."""
    ...
    broadphase_filter: (
        tuple[Literal["plane", "sphere", "aabb", "obb"], ...] | None
    ) = None
    ...
    **Important:** A captured graph holds pointers to the GPU arrays that existed
    ...
    def _finish_init(self) -> None:
        ...
        self.nan_guard = NanGuard(
            self.cfg.nan_guard, self.num_envs, self._mj_model
        )
    ...
    def expand_model_fields(self, fields: tuple[str, ...]) -> None:
        ...
        if self._sensor_context is not None:
            self._sensor_context.recreate(
                self._mj_model, self._expanded_fields
            )
    ...
    def step(self) -> None:
        with (
            wp.ScopedDevice(self.wp_device),
            self.nan_guard.watch(self.data),
        ):
