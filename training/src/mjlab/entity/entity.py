...
class EntityCfg:
    ...
    class InitialStateCfg:
        ...
        joint_pos: dict[str, float] | None = field(
            default_factory=lambda: {".*": 0.0}
        )
        joint_vel: dict[str, float] = field(
            default_factory=lambda: {".*": 0.0}
        )
  ...
  """
    ...
    collisions: tuple[spec_cfg.CollisionCfg, ...] = field(
...
class Entity:
    ...
    def _build_spec(self) -> None:
        from mjlab.entity.variants import (
            VariantEntityCfg,
            build_merged_variant_spec,
        )

        if isinstance(self.cfg, VariantEntityCfg):
            self._spec, self._variant_metadata = build_merged_variant_spec(
                self.cfg
            )
        else:
    ...
    def _identify_joints(self) -> None:
        ...
        if (
            self._all_joints
            and self._all_joints[0].type == mujoco.mjtJoint.mjJNT_FREE
        ):
    ...
    def _add_actuators(self) -> None:
        ...
        pending: list[
            tuple[actuator.ActuatorCfg, actuator.Actuator, list[str]]
        ] = []
        for actuator_cfg in self.cfg.articulation.actuators:
            ...
            try:
                if actuator_cfg.transmission_type == TransmissionType.JOINT:
                    ...
                    target_spec_names = [
                        self._spec.tendons[i].name for i in target_ids
                    ]
                    ...
                    target_spec_names = [
                        self.spec.sites[i].name for i in target_ids
                    ]
                else:
                    raise TypeError(
            ...
            actuator_instance = actuator_cfg.build(
                self, target_ids, target_names
            )
            self._actuators.append(actuator_instance)
            pending.append(
                (actuator_cfg, actuator_instance, target_spec_names)
            )
        ...
        flat.sort(
            key=lambda item: (
                ...
                order_maps[item[0].transmission_type].get(
                    item[2], float("inf")
                ),
            )
        )
    ...
    def _add_initial_state_keyframe(self) -> None:
        ...
        key_qpos = (
            np.hstack(qpos_components) if qpos_components else np.array([])
        )
    ...
    def find_materials(
        ...
        return resolve_matching_names(
            name_keys, material_subset, preserve_order
        )
    ...
    def initialize(
        ...
        builtin_group, custom_actuators = BuiltinActuatorGroup.process(
            self._actuators
        )
        ...
        if self.is_articulated:
            if self.cfg.init_state.joint_pos is None:
                ...
                default_joint_pos = torch.tensor(
                    key_qpos[nq_root:], device=device
            ...
            default_joint_vel = torch.tensor(
                resolve_expr(
                    self.cfg.init_state.joint_vel, self.joint_names, 0.0
                ),
                device=device,
            )[None].repeat(nworld, 1)
            ...
            unlimited = ~torch.tensor(
                mj_model.jnt_limited[joint_ids_list],
                device=device,
                dtype=torch.bool,
            )
            for limits in (
        ...
        if self.is_actuated:
            ...
            joint_pos_target = torch.empty(
                nworld, 0, dtype=torch.float, device=device
            )
            joint_vel_target = torch.empty(
                nworld, 0, dtype=torch.float, device=device
            )
        ...
        if self.has_tendon_actuators:
            ...
            tendon_len_target = torch.empty(
                nworld, 0, dtype=torch.float, device=device
            )
            tendon_vel_target = torch.empty(
                nworld, 0, dtype=torch.float, device=device
            )
    ...
    def _build_default_root_state(
        self, nworld: int, device: str
    ) -> torch.Tensor:
    ...
    def _compute_indexing(
        ...
        body_ids = torch.tensor(
            [b.id for b in bodies], dtype=torch.int, device=device
        )
        geom_ids = torch.tensor(
            [g.id for g in geoms], dtype=torch.int, device=device
        )
        site_ids = torch.tensor(
            [s.id for s in sites], dtype=torch.int, device=device
        )
        tendon_ids = torch.tensor(
            [t.id for t in tendons], dtype=torch.int, device=device
        )
        cam_ids = torch.tensor(
            [c.id for c in cameras], dtype=torch.int, device=device
        )
        light_ids = torch.tensor(
        ...
        pair_ids = torch.tensor(
            [p.id for p in pairs], dtype=torch.int, device=device
        )
        joint_ids = torch.tensor(
            [j.id for j in joints], dtype=torch.int, device=device
        )
