...
class EntityData:
    ...
    def write_joint_state(
        ...
        if not self.is_articulated:
            raise ValueError(
                "Cannot write joint state for non-articulated entity."
            )
    ...
    def write_joint_position(
        ...
        if not self.is_articulated:
            raise ValueError(
                "Cannot write joint position for non-articulated entity."
            )
    ...
    def write_joint_velocity(
        ...
        if not self.is_articulated:
            raise ValueError(
                "Cannot write joint velocity for non-articulated entity."
            )
    ...
    def write_mocap_pose(
        ...
        env_ids = self._resolve_env_ids(env_ids)
        self.data.mocap_pos[env_ids, self.indexing.mocap_id] = pose[
            :, 0:3
        ].unsqueeze(1)
        self.data.mocap_quat[env_ids, self.indexing.mocap_id] = pose[
            :, 3:7
        ].unsqueeze(1)
    ...
    def root_link_pose_w(self) -> torch.Tensor:
        ...
        quat_w = self.data.xquat[
            :, self.indexing.root_body_id
        ]  # (num_envs, 4)
    ...
    def geom_vel_w(self) -> torch.Tensor:
        ...
        body_ids = self.model.geom_bodyid[
            self.indexing.geom_ids
        ]  # (num_geoms,)
        subtree_com = self.data.subtree_com[:, self.indexing.root_body_id]
    ...
    def site_vel_w(self) -> torch.Tensor:
        ...
        body_ids = self.model.site_bodyid[
            self.indexing.site_ids
        ]  # (num_sites,)
        subtree_com = self.data.subtree_com[:, self.indexing.root_body_id]
        cvel = self.data.cvel[:, body_ids]
        return compute_velocity_from_cvel(pos, subtree_com.unsqueeze(1), cvel)
