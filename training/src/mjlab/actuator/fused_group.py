...
class FusedActuatorGroup:
        ...
        for group in self._groups:
            ...
            for name in group.actuator_type.param_names:
                group.params[name] = torch.cat(
                    [
                        _require(getattr(a, name))
                        for a in group.absorbed_actuators
                    ],
                    dim=1,
                )
            offset = 0
            for act in group.absorbed_actuators:
                ...
                for name in group.actuator_type.param_names:
                    setattr(
                        act, name, group.params[name][:, offset : offset + n]
                    )
                offset += n
    ...
    def apply_controls(self, data: EntityData) -> None:
        ...
        for group in self._groups:
            pos_attr, vel_attr, eff_attr, cur_pos_attr, cur_vel_attr = (
                _FIELD_MAP[group.transmission_type]
            )
