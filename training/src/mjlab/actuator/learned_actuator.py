...
class LearnedMlpActuator(DcMotorActuator[LearnedMlpActuatorCfg]):
    ...
    def initialize(
        ...
        self.network = torch.jit.load(
            self.cfg.network_file, map_location=device
        )
    ...
    def compute(self, cmd: ActuatorCmd) -> torch.Tensor:
        ...
        pos_inputs = [
            self._pos_error_history[lag]
            for lag in range(self.cfg.history_length)
        ]
        vel_inputs = [
            self._vel_history[lag] for lag in range(self.cfg.history_length)
        ]
        ...
        if self.cfg.input_order == "pos_vel":
            network_input = torch.cat(
                [pos_flat * self.cfg.pos_scale, vel_flat * self.cfg.vel_scale],
                dim=1,
            )
        elif self.cfg.input_order == "vel_pos":
            network_input = torch.cat(
                [vel_flat * self.cfg.vel_scale, pos_flat * self.cfg.pos_scale],
                dim=1,
            )
        else:
            raise ValueError(
