    ...
    def __post_init__(self) -> None:
        ...
        if self.frictionloss is not None:
            assert self.frictionloss >= 0.0, (
                "frictionloss must be non-negative."
            )
        if self.viscous_damping is not None:
            assert self.viscous_damping >= 0.0, (
                "viscous_damping must be non-negative."
            )
        if self.transmission_type == TransmissionType.SITE and (
            (self.armature is not None and self.armature > 0.0)
            or (self.frictionloss is not None and self.frictionloss > 0.0)
            or (
                self.viscous_damping is not None and self.viscous_damping > 0.0
            )
        ):
            raise ValueError(
        ...
        assert 0.0 <= self.delay_hold_prob <= 1.0, (
            "delay_hold_prob must be in [0, 1]."
        )
...
def delay_command(
    ...
    """
    stacked = torch.stack(
        (position_target, velocity_target, effort_target), dim=-1
    )
...
class Actuator(ABC, Generic[ActuatorCfgT]):
    ...
    @staticmethod
    def control_law(
        params: dict[str, torch.Tensor], cmd: ActuatorCmd
    ) -> torch.Tensor:
    ...
    def initialize(
        ...
        global_to_local = {
            gid.item(): i for i, gid in enumerate(entity_ctrl_ids)
        }
    ...
    def get_command(self, data: EntityData) -> ActuatorCmd:
        ...
        if self.transmission_type == TransmissionType.JOINT:
            ...
            raise ValueError(
                f"Unknown transmission type: {self.transmission_type}"
            )
